      ...
      <div class="set-content" :style="{height: height - 99 + 'px'}">
        <div class="content">
          <div class="copy-wrapper">
            ...
            <div class="copy-des">{{taskDes}}</div>
          </div>
  ...
  export default {
    ...
    computed: {
      ...
      freezeMoney () {
        ...
        if (refundLen === 2) {
          aRefund = [trueRefund, falseRefund]
        } else if (refundLen === 3) {
          aRefund = [trueRefund, falseRefund, homeFailRefund]
        }
        maxRefund = arrSort(aRefund)[0]
        return (this.amount * this.count) * maxRefund
      },
      taskDes () {
        return unescape(this.copyTaskData.task_content)
      }
    },
    methods: {
      ...mapActions([
