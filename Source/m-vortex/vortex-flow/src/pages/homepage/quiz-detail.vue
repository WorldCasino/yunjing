  ...
  <f7-page navbar-fixed toolbar-fixed pull-to-refresh @ptr:refresh="onRefresh" ref="quizDetailRef" class="page-quiz-detail">
    <f7-navbar sliding class="nav-custom">
      ...
      <f7-nav-right>
        ...
        <div v-else class="more-action" @click="openShare">更多</div>
      </f7-nav-right>
    </f7-navbar>
...
//        socket: state => state.socket,
        ...
        shareData: state => state.quizDetail.shareData,
...
//        是否是发布者自己
      ...
      calcMineBetsAll: function () {
        return this.calcMineBets('all')
      },
      calcMineBetsCoin: function () {
        // 金币0，金豆1
        return this.calcMineBets(0)
      },
      calcMineBetsBean: function () {
        // 金币0，金豆1
        return this.calcMineBets(1)
      }
    },
    beforeCreate () {
...
//      重置微信分享的内容
    ...
    mounted () {
      ...
      var windowBack = () => {
        ...
        window.removeEventListener('popstate', windowBack, false)
      }
      window.addEventListener('popstate', windowBack, false)
...
//            this.$store.state.quizDetail.data.pics[0].pic_url = ''
//            console.log(temp)
//            setTimeout(function () {
//              self.$store.state.quizDetail.data.pics[0].pic_url = temp
//            }, 500)
            this.getShareJsSdkConf([window.location.href])

            if (this.taskSocket !== null) {
              console.log('task socket ready!')
              if (this.watches.indexOf(this.taskId) === -1) {
                this.watches.push(this.taskId)
              }
              this.taskSocket.emit('watch', JSON.stringify(this.watches))
            }
          } else {
      ...
      betShake: {
        handler: function (val) {
          ...
          if (self.currentCount === self.quizDetail.quantity) {
            // this.$store.quizDetail.data.pics[0] = ''
...
//            console.log(imgUrl)
//            console.log(desc)
            var shareUrl = servConf.WAP_ADDR + '?page=quiz_detail'
            if (self.token) {
...
//        我下注的总数量（包括金豆下的和金币下的）
        ...
        for (var i = 0; i < this.quizDetail.answer.length; i++) {
          ...
          for (var j = 0; j < this.quizDetail.gamblers.length; j++) {
            ...
            if (type === 'all') {
              // 如果没传type。则取所有下注数
              _type = gambler.coin_type
            }
            if (mineId === gambler.user_id && answer.answer_id === gambler.answer_id && gambler.coin_type === _type) {
        ...
        self.socket.on('message', function (data) {
          ...
          switch (json.type) {
            case 'message':
              ...
              // 高度要动态调整
              tempBox.style.height = '120px'
              divBox.appendChild(tempBox)
              // 显示该临时元素，起到滚动效果
              tempBox.scrollIntoView(false)
              // 删除该元素
...
//            未登录或者没有用户信息数据
      ...
      openShare () {
        /* if (this.quizDetail.task_type === 1 || this.quizDetail.task_type === 2 || this.quizDetail.task_type === 3) {
          console.log('此猜信息', this.quizDetail)
          // 复制足彩类的竞猜数据
        ...
        this.$store.state.quizDetail.isShowMore = true
        this.$store.state.quizDetail.isBanker = this.isBanker
      },
      subscribeWx () {
