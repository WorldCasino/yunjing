...
module.exports = function (req,res,next) {
    ...
    Q.fcall(function () {
        ...
        return Q.all([
            ...
            Q.ninvoke(mysql,'query',{
                sql:'select lottery_type, task_status, sale_price, quantity, settle_time FROM t_tasks WHERE task_id = ?',
                values:[id]
            }),
            Q.ninvoke(mysql,'query',{
                sql:'SELECT ord.user_id, ord.answer_id, ord.quantity, ord.create_date, ord.update_date, usr.nickname, usr.gender, usr.head_url ' +
            ...
            var status = parseInt(result[1][0][0].task_status);
            var quantity_count = parseInt(result[1][0][0].quantity);
            var salePrice = result[1][0][0].sale_price;
            var user_type = parseInt(result[4][0][0].user_type);
            var settle_time = result[1][0][0].settle_time;
            ...
            if(status === 30){
                res.pkg.data = result[0][0].map(function (currentValue) {
                    ...
                    //超时开奖，平份奖金
                    if (lottery_type === 1 && new Date(settle_time).getTime()/1000 < new Date().getTime()/1000){
                        coins =  coin_avg*(answerA.quantity+answerB.quantity);//+ userQty * salePrice ;
