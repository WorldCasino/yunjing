  ...
  export default {
    ...
    computed: {
      ...mapState({
        ...
        convertData: state => state.convertStoreItem.data,
        convertStatus: state => state.convertStoreItem.status,
        convertShake: state => state.convertStoreItem.shake,
        payData: state => state.convertStoreItem.payData,
        payStatus: state => state.convertStoreItem.payStatus,
        payShake: state => state.convertStoreItem.payShake
      }),
      ...mapGetters([
        'itemId',
        'storeItemData',
      ...
      convertShake: {
        handler: function (val) {
          if (this.convertStatus === null) {
            this.storeItemPay([this.convertData.order_id, this.convertData.order_code, this.convertData.cost])
          } else {
            this.$f7.addNotification({
              title: '出错啦',
              message: this.convertStatus.message
            })
          }
        }
      },
      payShake: {
        handler: function (val) {
          if (this.payStatus === null) {
            ...
            this.$f7.addNotification({
              title: '出错啦',
              message: this.payStatus.message
            })
          }
        }
      }
