<template>
  <div>
    <div class="list" v-on:touchend="onTouchEnd()">
      <div v-for="(quiz, index) in quizListFoot" class="list-wrapper">
  ...
  export default {
    ...
    computed: {
      ...mapState({
        ...
        status: state => state.quizListFoot.status,
        shake: state => state.quizListFoot.shake,

        // 取名为quizListFoot，为使用公共模块设定
        quizListFoot: state => state.quizListFoot.quizzes,

        // 滚动条高度
        scrollTop: state => state.quizListFoot.scrollTop
      })
    },
    methods: {
      ...mapActions([
