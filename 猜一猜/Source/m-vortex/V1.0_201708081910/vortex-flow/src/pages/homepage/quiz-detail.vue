  ...
  <f7-page navbar-fixed toolbar-fixed pull-to-refresh @ptr:refresh="onRefresh" ref="quizDetailRef" class="page-quiz-detail">
    ...
    <div class="box-info" v-if="quizDetail">
      ...
      <Options :total="quizDetail.quantity" :count="currentCount" :is-published="isPublished" :showDiscuss=false :discuss="quizDetail.message_count" :options="quizDetail.answer"
               :show-detail=true :mine-bets="calcMineBets" :is-banker="isBanker" :taskId="taskId" :gold="quizDetail.sale_price" @bet="bet"></Options>
...
//            console.log(desc)
      ...
      confirmResult: {
        handler: function (val) {
          ...
          } else if (this.confirmContext === 'quiz_detail_bet') {
            // this.showBetConfirm = false
            this.$store.state.betTip = this.isDefaultSelect
            StorageHelper.saveBetTip(this.isDefaultSelect)
...
//        }
        ...
        if (this.showBetTip === true) {
          ...
          this.betValue = value
          this.$store.state.confirm.context = 'quiz_detail_bet'
          this.$store.state.confirm.isConfirm = true
          ...
          this.$store.state.confirm.isBet = true
          this.$store.state.confirm.title = ''
          this.$store.state.confirm.text = ''
          this.$store.state.confirm.contextData = [this.task_id, value[0].answer_id, 1]
          // 此按钮的赔率,选项，答案
          this.$store.state.confirm.betData.odds = value[0].odds
          this.$store.state.confirm.betData.answer = value[0].answer
          this.$store.state.confirm.betData.letter = value[1]
          this.$store.state.confirm.betData.salePrice = this.quizDetail.sale_price
        } else {
          self.quizBet([self.taskId, value[0].answer_id, 1])
        }
      },
