...
//        let falseInout = document.getElementsByClassName('result-input-false').item(0)
        ...
        if (this.token !== null) {
          ...
          console.log(isNext)
          if (isNext === 1) {
            this.$f7.alert('竞猜描述字数请大于' + this.textMin + '个', '提示')
          } else if (isNext === 2) {
            this.$f7.alert('请上传1张答案图', '提示')
          } else if (isNext === 3) {
            this.$f7.alert('请上传1-3张参考图', '提示')
          } else if (isNext === 4) {
            this.$f7.alert('请填写正确答案', '提示')
          } else if (isNext === 5) {
            this.$f7.alert('请填写错误答案', '提示')
          } else if (isNext === 6) {
            this.$f7.alert('正确答案和错误答案不能一致', '提示')
          } else if (isNext === 10) {
