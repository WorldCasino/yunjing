...
public class BasketballMatchUtil2 {
...
//                System.out.println(object.get("result"));
                ...
                for (int i = 0; i < jsonArray.size() ; i++) {
                    ...
                    for (int j = 0; j < jsonArray2.size() ; j++) {
                         ...
                         BallUtil.SaveMatch( home_team_name, away_team_name,  open_time, title, MatchesTypeEnum.Basketball.getCode()+"" ,"");
                     }
               }
             }else{
        ...
        try {
            ...
            if(object.getInteger("error_code")    ==0){
                ...
                for (int i = 0; i < jsonArray.size() ; i++) {
                    ...
                    JSONObject jsonObject2=jsonArray.getJSONObject(i);
                    String time_title=  jsonObject2.getString("title");
                    String day_date2=time_title.substring(0,5).trim();
                    String day_week=time_title.substring(6,time_title.length()).trim();
                    if(!day_date2.equals(day_date)){
                        continue;
                    }
                    //获取赛事
                    JSONArray jsonArray2= jsonObject2.getJSONArray("tr");
                    for (int j = 0; j < jsonArray2.size() ; j++) {
                        JSONObject jsonObject3= jsonArray2.getJSONObject(j);
                        String time=jsonObject3.getString("time");
                        String day_time2=time.substring(6,time.length());
                        String home_team_name2=jsonObject3.getString("player2");
                        String away_team_name2=jsonObject3.getString("player1");
                        if(day_time2.equals(day_time) && home_team_name.equals(home_team_name2) && away_team_name.equals(away_team_name2)){
                            String score=jsonObject3.getString("score");
                            int status=jsonObject3.getIntValue("status");
                            String home_score="0";
                            String away_score="0";
                            if(!score.equals("VS")){
                                home_score=score.split("-")[1];
                                away_score=score.split("-")[0];
                            }
