...
//            LOTTERY_LOGGER.error(String.format("竞猜项目【%s】不存在，不能开奖！",taskId));
            throw new ApiException(ErrorCodeEnum.SystemError.getCode(),String.format("竞猜项目【%s】不存在，不能开奖！",taskId));
        }
...
//                                int userTotalQty = 0;
     ...
     * @throws ApiException
     */
    public List<Long> selectWattingScoreTasks() throws ApiException{
        return taskMapper.selectWattingScoreTasks();
    }

    /**
     * 检查所有待处理比分的竞猜（倒计时 未及时处理的）
     * @return
     * @throws ApiException
     */
    public void dealScoreProccess(long taskId) throws ApiException{
        Map<String,Object> match = taskMapper.selectMatch(taskId);
        long matchId= (long) match.get("match_id");
        int matchType= (int) match.get("match_type");
        int playType= (int) match.get("play_type");


        //判断比赛是否结束
        int status=taskMapper.selectMatchStatus(taskId);
        if(status!=3){
            //比赛未结束，返回队列,设置过期时间3分钟
            pushScoreQueue(taskId,60*3);
            return ;
        }

        //判断是否设置过正确答案
        int cnt=taskMapper.selectRightAnswer(taskId);
        if(cnt!=0){
            //已处理，竞猜已设置答案
            return;
        }
        ...
        //获取主客队比分
        Map<String,Object> score=taskMapper.selectScore(matchId);
        ...
        taskAnswerMapper.setRightAnswer(params);
    }
     ...
     * @param expire 倒计时时间（秒）
     */
    public void pushScoreQueue(long taskId,int expire){
        ...
        try {
            ...
            e.printStackTrace();
            //REDIS提交失败 不能影响业务流程
            LOTTERY_LOGGER.error(String.format("竞猜项目【%s】成功加入比赛结果待处理队列, 过期时间%s秒，原因：%s",taskId,expire, e.getMessage()),e);
        }
    }
