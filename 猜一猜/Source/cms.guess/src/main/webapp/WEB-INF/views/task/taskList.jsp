...
<div class="content">
    ...
    <div class="row-fluid">
        <form id="queryForm" class="form-horizontal" action="" method="post">
            ...
            <input type="text" name="lookType" class="form-control" style="display: none;"/>
            <div class="form-group">
                <label for="create_date" class="col-sm-1 control-label">发布时间</label>
                <div class="col-sm-2" >
...
<script>
    $(function () {
        ...
        var type = cicada.getQueryString("lookType");
        $("input[name=lookType]").val(type);
        if(type != null && type != ""){
            $(".caozuo").hide();
        }
        ...
        var tables = $("#dataTable").dataTable({
            ...
            ajax: {
                    type: "post",
                    url: cicada.CONSTS.URL_BASE_API+'task/list',
                    dataSrc: "data",
                    dataType:'json',
...
//                {"data": 'saleAmount'},
                ...
                },
                {
                    "data": 'nickName',
                    "render": function (data,type,rowData,callback) {
                        var pubuser = cicada.isNullOrEmpty(data) ? rowData.userPhone :
                            (cicada.isNullOrEmpty(rowData.userPhone)?data:data+"<br/>"+rowData.userPhone);
...
//删除
        ...
        $("#dataTable tbody").on("click", "#btnDetail", function () {
            var data = tables.api().row($(this).parents("tr")).data();
            var taskType=data.taskType.code;
            var url = 'task/view?taskId='+data.taskId;
            //var url = 'task/detail?taskId='+data.taskId+'&data.taskType='+data.taskType;
            var id = "task_"+data.taskId;
            var title = "竞猜详情【"+unescape(data.taskContent)+"】";
