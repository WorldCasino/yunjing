...
<head>
    ...
    <script src="<%=request.getContextPath()%>/baidu/js/lushu.js"></script>
    <title>鼠标绘制工具</title>
</head>
<body>
...
<div id="result">
    <dl>
        ...
        <ul>
            <li>
                <label><input type="radio" name="openClose1" onclick="drawingManager.open()"/>打开</label>
                <label><input type="radio" name="openClose1" onclick="drawingManager.close()" checked="checked"/>关闭</label>
            </li>
        </ul>
        <dt>是否进行线或面积的计算(单位米)</dt>
        <ul>
            <li>
                <label><input type="radio" name="openClose" onclick="drawingManager.enableCalculate()"/>打开</label>
                <label><input type="radio" name="openClose" onclick="drawingManager.disableCalculate()" checked="checked"/>关闭</label>
            </li>
        </ul>
        <dt>绘制功能</dt>
        <dd>
    ...
    <dd>
        <ul>
            <li>
                <input style="width: 200px;height: 30px" type="text" id="Lng1" placeholder="经度">
                <br>
                <input style="width: 200px;height: 30px" type="text" id="Lat1" placeholder="纬度">
                <br>
                <button style="width: 60px;height: 30px" onclick="move()">移动</button>
                <button style="width: 60px;height: 30px" onclick="real()">恢复</button>
            </li>
        </ul>
    </dd>
    <dt>路书</dt>
...
<script type="text/javascript">
    ...
    //用经纬度设置地图中心点
    function move() {
        if (document.getElementById("Lng1").value != "" && document.getElementById("Lat1").value != "") {
            var new_point = new BMap.Point(document.getElementById("Lng1").value, document.getElementById("Lat1").value);
            map.panTo(new_point)
        } else {
            alert("请输入坐标")
        }
    ...
    var poi = new BMap.Point(120.256877, 31.548735); //创建中心坐标点
    map.centerAndZoom(poi, 10);     // 初始化地图，设置中心点坐标和地图级别
    ...
    var content = '<div style="margin:0;line-height:20px;padding:2px;">' +
            '地址：滨湖区建筑西路599号<br/>电话：(0510)8888888<br/>简介：无锡国家工业设计园。' +
            '</div>';
        ...
        if (e.drawingMode == BMAP_DRAWING_POLYLINE || e.drawingMode == BMAP_DRAWING_POLYGON || e.drawingMode == BMAP_DRAWING_RECTANGLE) {
            ...
            for(var i=0;i<path.length;i++){
              if(e.drawingMode == BMAP_DRAWING_POLYLINE) {
                  ...
                  points = new BMap.Point(path[i].lng,path[i].lat)
...
//
    ...
    $("run").onclick = function(){
        ...
        defaultContent:"",
        autoView:true,  //是否开启视野自动调整
        icon:new BMap.Icon("<%=request.getContextPath()%>/baidu/image/car.png",new  BMap.Size(52,26),{anchor:new BMap.Size(27,13)}),
        enableRotation:true,    //是否设置marker随着道路走向进行旋转
        speed:16000,
        landmarkPois:[]
    });
            //console.log(allPoints )
    //        map.panTo(new BMap.Point(116.411053, 39.950507))
            lushu.start()
//        allPoints = []
        }
