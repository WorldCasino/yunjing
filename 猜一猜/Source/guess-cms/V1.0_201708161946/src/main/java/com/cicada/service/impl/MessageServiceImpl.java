    ...
    public boolean sendLotteryMessage(LotteryResultVo resultVo) throws ApiException{
        ...
        try {
            ...
            if(!StringUtils.isEmpty(dealer.getWxOpenId())) {
                ...
                param.append("累计收入：" + new Double(resultVo.getIncomeTotal()).intValue() + "金币\r\n")
                        .append("累计打赏：" + new Double(resultVo.getExpendTotal()).intValue() + "金币\r\n")
                        .append("累计盈亏：" + new Double(resultVo.getIncomeTotal() - resultVo.getExpendTotal()).intValue() + "金币\r\n")
            ...
            if(!StringUtils.isEmpty(dealer.getDeviceToken())){
                String content = "您发布的竞猜开奖了，点我查看有多少人猜对了》";
            ...
            for (LotteryResultVo.PlayerVo playerVo:list) {
                ...
                if(!StringUtils.isEmpty(client.getWxOpenId())){
                    ...
                    for (LotteryResultVo.BettingVo bet:playerVo.getBettingVoList()) {
                        param.append(String.format("参与答案%s：%s注【%s倍】%s金币\r\n",
                                bet.getAbc(),bet.getQty(),bet.getOdds(),new Double(bet.getCoins()).intValue()));
                        if(bet.isRight()) totalCoins += bet.getCoins();
                        else totalCoins -= bet.getCoins();
                    }
                    param.append("累计盈亏：" + new Double(totalCoins).intValue() + "金币\r\n");
