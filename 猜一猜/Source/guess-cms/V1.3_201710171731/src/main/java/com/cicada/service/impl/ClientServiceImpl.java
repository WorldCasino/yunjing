...
import com.cicada.mapper.TaskOrderMapper;
...
//                String nickName = EscapeUnescape.escape(userInfo.getNickName());
    ...
    public int updateWechatUnionId(long userId,String wxUnionId,String wxOpenId) throws ApiException{
        Map<String,Object> params = new HashMap<>();
        params.put("userId",userId);
        params.put("wxUnionId",wxUnionId);
        params.put("wxOpenId",wxOpenId);

        return userMapper.updateWechatUnionId(params);
    }

    //登录成功，将登录操作存入操作日志表，并添加进操作队列
    public void operateLogin(ClientEntity user){
        OperateVo operateVo=new OperateVo();
        operateVo.setUserId(user.getUserId());
        operateVo.setOperateType(1);
        operateVo.setQuantity(1);
        operateVo.setOperateTime(new Timestamp((new Date()).getTime()));
        operateService.insert(operateVo);
