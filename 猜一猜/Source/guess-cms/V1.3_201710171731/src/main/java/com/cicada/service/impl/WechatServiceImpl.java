...
public class WechatServiceImpl implements IWechatService {
    ...
    private final static Map<String,WeixinProxy> weixinProxyApp;
    ...
    static {
        ...
        weixinProxyApp = new HashMap<>();

        try{
            //APP应用
            String wxConfigStr = Weixin4jConfigUtil.getValue("account");
            JSONObject wxConfig = JSONObject.parseObject(wxConfigStr);
            String components= wxConfig.getString("components");
            if(!StringUtils.isEmpty(components)) {
                List<WeixinAccount> list = new ArrayList<>();
                list = JSONArray.parseArray(components, WeixinAccount.class);
                for (WeixinAccount x : list) {
                    WeixinProxy proxy = new WeixinProxy(x, new FileCacheStorager<>());
                    weixinProxyApp.put(x.getId(),proxy);
                }
            }
        }
        catch (Exception e){
...
//            return false;
    ...
    public User getWechatUserInfo(String openId){
        User user = null;
        try {
            user = weixinProxy.getUser(openId);
        } catch (WeixinException e) {
            e.printStackTrace();
        }
        return user;
     ...
     * @throws ApiException
     */
    public User getWechatUserInfo(String oauthCode,String appId) throws ApiException{
        User wechatUser;
        try {
            OauthApi oauthApi;
            if(StringUtils.isEmpty(appId)) {
                oauthApi = weixinProxy.getOauthApi();
            }else if(weixinProxyApp.containsKey(appId)){
                oauthApi = weixinProxyApp.get(appId).getOauthApi();
            }else{
                throw new ApiException(ErrorCodeEnum.WechatRequestError.getCode(),"微信授权失败,请求的APP_ID不存在");
            }
    ...
    public String sendLotteryMessage(String openId,boolean publishUser,long taskId,String items) throws ApiException{
        ...
        try {
            ...
            msg.pushHead(publishUser? "你发布的竞猜项目已开奖！":"你参与的竞猜项目已开奖！")
                    .pushTail("查看详情》")
