...
module.exports = function (req, res, next) {
    ...
    Q.fcall(function () {
        ...
        // 判读金币是否充足
        return Q.ninvoke(mysql, 'query', {
            sql: 'SELECT account_id, coin_balance FROM t_user_account WHERE coin_type = 0 and user_id = ?',
            values: [user_id]
        }).then(function (result) {
            var rows = result[0];
            pd.checkState(rows.length === 1, '金币不足,不能兑换！',1001);
            pd.checkState(rows[0].coin_balance > parseFloat(req.body.cost), '金币不足,不能兑换！',1001);
            var accountId = result[0][0].account_id;
            return accountId;
        });
        ...
        return Q.fcall(function () {
            // 更新统计表
            return Q.ninvoke(mysql, 'query', {
                sql: 'UPDATE t_user_account SET coin_balance = coin_balance - ? WHERE coin_type = 0 and user_id = ?',
