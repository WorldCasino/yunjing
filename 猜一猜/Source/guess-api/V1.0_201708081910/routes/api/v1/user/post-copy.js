    ...
    Q.fcall(function () {
        ...
        return Q.fcall(function () {
            ...
            return Q.ninvoke(mysql,'query',{
                ...
                //转发的始终以第一个竞猜为主
                if(orgTask.parent_id>0) params.parentId = orgTask.parentId;
        ...
        return Q.fcall(function () {
            ...
            */
            ...
            return Q.ninvoke(mysql, 'query', {
                sql:'insert into t_tasks(user_id,task_title,task_type,task_content,sale_price,quantity,sale_amount,task_status,hot,lock_time,settle_time,parent_id,locked_coins,create_by) '+
                'select ?,task_title,task_type,task_content,?,?,?,task_status,0,lock_time,settle_time,?,?,? '+
                'from t_tasks t '+
                'where t.task_id = ? ; ',
                values: [params.user_id,quantity,price,quantity * price,params.parentId,params.lockedBalance,params.user_id,params.parentId]
            }).then(function (results) {
            ...
            return Q.all([
                ...
                params.orgAnswerList = results[2][0];
            });
