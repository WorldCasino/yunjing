...
module.exports = function (req, res, next) {
    ...
    Q.fcall(function () {
        ...
        return Q.all([
            ...
            pd.checkState(answers[0].cnt === 1, '答案编号不正确');
            pd.checkState(status === 20, '已开奖或已经删除');
            pd.checkState((myLeft - locked) >= (quantity * price), '账户金币不足',1001);
            pd.checkState(quantity <= (total - sold), '超过了项目最大买入值');
            pd.checkState(lockTime == null || lockTime > new Date(), '竞猜下注已锁定');
        });
    }).then(function () {
        //console.log('step3');
