...
public class UserAccountDetailService extends ServiceImpl<UserAccountDetailMapper,UserAccountDetailEntity> implements IUserAccountDetailService{
    ...
    public boolean applyWithdraw(UserEntity user,double amount,long cardId,String captcha) throws ApiException{
         ...
         */
        try {
             GameRsp<Object> gameRsp= gameConnector.downScore(user.getKyAccount(),String.valueOf(amount),orderid);
             DBack dBack= JSON.parseObject(JSONObject.toJSONString(gameRsp.getD()), DBack.class);
             if( dBack.getCode() != 0) {
    ...
    public void UpdateAccountDetail(String tradeId, String  examine, String playMoeny,  String updateBy,String remarks) {
        ...
        if(examine != null){
            ...
            double now_money=0d;
            try {
                GameRsp<Object> gameRsp= gameConnector.searchBalance(userEntity.getKyAccount());
                DBack dBack= JSON.parseObject(JSONObject.toJSONString(gameRsp.getD()), DBack.class);
                if(dBack != null && dBack.getCode() == 0) {
                    now_money = dBack.getMoney();
                }
            ...
            if(examine.equals("false")){
                 ...
                 */
                try {
                    GameRsp<Object> gameRsp= gameConnector.upScore(userEntity.getKyAccount(),String.valueOf(amount),orderid);
                    DBack dBack= JSON.parseObject(JSONObject.toJSONString(gameRsp.getD()), DBack.class);
                    if( dBack.getCode() != 0) {
                        throw new ApiException(ErrorCodeEnum.BgBalanceTransferException);
                    }
