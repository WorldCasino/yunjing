...
public class UserService extends ServiceImpl<UserMapper,UserEntity> implements IUserService{
    ...
    private static String touristMoney="100";//游客登录获取到的金额
...
//                jedis.del(device);
         ...
         * 开元平台注册
         */
        GameRsp<Object> gameRsp=gameConnector.login( kyAccount,touristMoney,ip,StringUtils.lineCode,KindIdEnum.Hall.getCode());
        DBack dBack= JSON.parseObject(JSONObject.toJSONString(gameRsp.getD()), DBack.class);
        if(dBack.getCode() == 0){
            url=dBack.getUrl();
        }else{
     ...
     */
    @Override
    public String getLoginCount(String device) throws ApiException {
        String loginCount="0";
        Jedis jedis = null;
        try{
            jedis = RedisHelper.getJedis();
            if(jedis.exists(device)){
                loginCount=jedis.get(device);
            }
        }catch (Exception e){
            RedisHelper.returnBrokenResource(jedis);
        }finally {
    ...
    public Double refreshUserBalance(long userId){
        ...
        try {
             ...
             * 获取开元账号余额
             */
            GameRsp<Object> gameRsp= gameConnector.searchBalance(user.getKyAccount());
            DBack  dBack= JSON.parseObject(JSONObject.toJSONString(gameRsp.getD()), DBack.class);
            if(dBack.getCode() == 0){
                balance=dBack.getMoney();
            }
            //判断
...
//            }
    ...
    public String getUrlByKind(long userId,String ip,String kind_id) throws Exception {
         ...
         * 开元平台获取游戏大厅地址
         */
        GameRsp<Object> gameRsp=gameConnector.login( kyAccount,"0",ip,StringUtils.lineCode,Integer.parseInt(kind_id));
        DBack dBack= JSON.parseObject(JSONObject.toJSONString(gameRsp.getD()), DBack.class);
        if(dBack.getCode() == 0){
            url=dBack.getUrl();
        }else{
