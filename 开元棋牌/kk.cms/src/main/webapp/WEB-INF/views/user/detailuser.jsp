...
<script>
    var id = kk.getQueryString("userId");
    window.userId = id;
    ...
    $(function () {
        ...
        function loadForm() {
             //获取玩家详情
            kk.post('user/system/findById',JSON.stringify({userId :  window.userId}),function (result) {
//                console.log(result);
                bindFormData(result);
                //TODO
            },function (err) {
...
//            $(".head_url").attr("alt",usepd.nickname);
            ...
            if(imgurl == "" || imgurl == null){
                ...
                imgurl=kk.CONSTS.URL_BASE_IMG+imgurl;
            }
            ...
            $(".userId").val(usepd.userId);
            $(".nickName").val(usepd.nickName);
            $(".agentId").val(usepd.agentNumber);

            $(".createDate").val( kk.timeStamp2String(usepd.createDate));
            $(".updateDate").val(kk.timeStamp2String(usepd.updateDate));
            //银行卡
...
//            $(".actionTotle").val(usepd.actionTotle);
        ...
        $("#editForm").bootstrapValidator({
            ...
            layer.confirm('确定给TA充值吗？', {
                ...
                $.ajax({
                    type: "POST",
                    url: kk.CONSTS.URL_BASE_API+"user/system/updateUserBalance",
                    data: {
        ...
        $("#listrecharge").on("click", function () {
            var url = 'ss_ac_detail/manager?userId='+window.userId+'&tradeKind=1';
        ...
        $("#listaction").on("click", function () {
            var url = 'ss_games/dataGrid?userId='+$(".userId").val();
...
//                    }
            ...
            $.ajax({
                type: "POST",
                url: kk.CONSTS.URL_BASE_API+"user/system/updateUserSomeInfor",
                data: {
                    "userId"   : window.userId ,
                    "phone"     : $(".phone").val(),
